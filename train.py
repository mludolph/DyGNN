...
def get_args():
    ...
    parser.add_argument(
        "-l", "--learning_rate", type=float, help="learning_rate", default=0.001
    )
    parser.add_argument(
        "-nn", "--num_negative", type=int, help="num_negative", default=5
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-ip", "--if_propagation", type=int, help="if_propagation", default=1
    )
    parser.add_argument(
        "-ia", "--is_att", type=int, help="use attention or not", default=1
    )
    parser.add_argument("-w", "--w", type=float, help="w for decayer", default=2)
    parser.add_argument("-s", "--seed", type=int, help="random seed", default=0)
    parser.add_argument(
        "-rp", "--reset_rep", type=int, help="whether reset rep", default=1
    )
    parser.add_argument(
        "-dc", "--decay_method", type=str, help="decay_method", default="log"
    )
    ...
    parser.add_argument(
        ...
        "--if_updated",
        type=int,
        help="use updated representation in loss",
        default=0,
    )
    parser.add_argument(
        "-wd", "--weight_decay", type=float, help="weight decay", default=0.001
    )
    parser.add_argument(
        "-nt",
        "--if_no_time",
    ...
    parser.add_argument(
        ...
        "--threhold",
        type=float,
        help="the threhold to filter the neighbors, if None, do not filter",
        default=None,
    )
    parser.add_argument(
        "-2hop", "--second_order", type=int, help="whether to use 2-hop prop", default=0
    )
...
def get_loss(data, head_reps, tail_reps, device):
    ...
    scores = torch.bmm(
        head_tensors.view(len(head_list), 1, head_tensors.size()[1]),
        tail_tensors.view(len(head_list), head_tensors.size()[1], 1),
    ).view(len(head_list))
...
def rank(
    node, true_candidate, node2candidate, node_reps, candidate_reps, device, pri=False
):
...
def train(args, data, num_nodes, model_save_dir):
    ...
    optimizer = torch.optim.Adam(
        filter(lambda p: p.requires_grad, dyGnn.parameters()),
        lr=learning_rate,
    ...
    for epoch in range(num_iter):
        ...
        for i, interactions in enumerate(data_loader):
            ...
            if i % x == 0:
                ...
                if transfer:
                    head_reps = nn.Embedding.from_pretrained(
                        dyGnn.transfer2head(dyGnn.node_representations.weight)
                    )
                    tail_reps = nn.Embedding.from_pretrained(
                        dyGnn.transfer2tail(dyGnn.node_representations.weight)
                    )
                else:
            ...
            if i % y == -1:
                ...
                head_ranks, tail_ranks, not_in_use, not_in_use2 = get_ranks(
                    validation_data,
                    head_reps,
                    tail_reps,
                    device,
        ...
        get_loss(validation_data, head_reps, tail_reps, device)
        ...
        if mean_head_rank < old_head_rank or mean_tail_rank < old_tail_rank:
            model_save_path = (
                model_save_dir + "/" + "model_after_epoch_" + str(epoch) + ".pt"
            )
            ...
            with open(model_save_dir + "/" + "0valid_results.txt", "a") as f:
                ...
                f.write(
                    ...
                    + " ; "
                    + "tail_rank var: "
                    + str(np.var(tail_ranks_numpy))
                    + "\n"
                )
                f.write(
                    "head_rank HITS 100: " + str((head_ranks_numpy <= 100).sum()) + "\n"
                )
                f.write(
                    "tail_rank_HITS 100: " + str((tail_ranks_numpy <= 100).sum()) + "\n"
                )
                f.write(
                    "head_rank HITS 50: " + str((head_ranks_numpy <= 50).sum()) + "\n"
                )
                f.write(
                    "tail_rank_HITS 50: " + str((tail_ranks_numpy <= 50).sum()) + "\n"
                )
                f.write(
                    "head_rank HITS 20: " + str((head_ranks_numpy <= 20).sum()) + "\n"
                )
                f.write(
                    "tail_rank_HITS 20: " + str((tail_ranks_numpy <= 20).sum()) + "\n"
                )
                f.write(
                    "============================================================================\n"
                )
