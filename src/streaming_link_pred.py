...
from src.datasets import Temporal_Dataset
from src.model_recurrent import DyGNN
from src.train import get_node2candidate, get_previous_links, rank
...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        help="emb_file",
        default="Dataset/UCI_email_1899_59835/opsahl-ucsocial/out.opsahl-ucsocial_line_200_64.emb",
    )
    parser.add_argument(
        "-emb2", "--emb_file2", type=str, help="emb_file for graphsage", default=None
    )
    parser.add_argument(
        "-emb3", "--emb_file3", type=str, help="emb_file for DynGEM", default=None
    )
    parser.add_argument(
        "-m", "--model_file", type=str, help="DyGNN model file", required=True
    )
    parser.add_argument(
        "-tr", "--train_ratio", type=float, help="train_ratio", default=0.8
    )
    parser.add_argument(
        "-vr", "--valid_ratio", type=float, help="valid_ratio", default=0.01
    )
    parser.add_argument(
        "-ter", "--test_ratio", type=float, help="test_ratio", default=0.1
    )
    parser.add_argument("-line", "--line", type=int, help="line or not", default=0)
    parser.add_argument("-p", "--pri", type=int, help="print rank or not", default=0)
    parser.add_argument(
        "-trans",
        "--transfer",
        type=int,
        help="transfer to head, tail representations",
        default=0,
    )
    parser.add_argument(
        "-data", "--dataset", type=str, help="which dataset to run", default="uci"
    )
    parser.add_argument("-w", "--w", type=float, help="w for decayer", default=1)
    parser.add_argument(
        "-ia", "--is_att", type=int, help="use attention or not", default=0
    )
    parser.add_argument(
        "-dc", "--decay_method", type=str, help="decay_method", default="exp"
    )
    parser.add_argument("-nor", "--nor", type=int, help="normalize or not", default=0)
    parser.add_argument(
        "-skip", "--skip", type=int, help="skip existing links", default=0
    )
    parser.add_argument(
        "-val", "--val_file", type=str, help="val_file for graphsage", default=None
    )
    parser.add_argument(
        "-val3", "--val_file3", type=str, help="val_file for dygem", default=None
    )
    parser.add_argument("-bo", "--bo", type=int, help="bo", default=1)
    parser.add_argument(
        "-recon", "--test_reconstruct", type=int, help="test_reconstruct", default=0
    )
    parser.add_argument(
        "-nt",
        "--if_no_time",
        type=int,
        help="if no time interval information",
        default=0,
    )
...
def prepare_interaction_time(interactions, num_nodes, device):
    ...
    recent_timestamp = torch.zeros(
        (num_nodes, 1), dtype=torch.float, requires_grad=False
    ).to(device)
...
def get_rank_new(
    ...
    if bo:
        if previous_links is not None:
            if (
                head_node in head_node2candidate
                and tail_node in tail_node2candidate
                and tail_node in head_node2candidate
                and head_node in tail_node2candidate
                and (head_node, tail_node) not in previous_links
            ):
                head_rank, head_length = rank(
                    head_node,
                    tail_node,
                    head_node2candidate,
                    head_reps,
...
def test(args, num_nodes, data):
    ...
    validation_data = data[
        int(len(data) * train_ratio) : int(len(data) * (train_ratio + valid_ratio))
    ]
    print(test_ratio)
    if int(len(data) * (train_ratio + valid_ratio + test_ratio)) > len(data):
        ...
        test_data = data[
            int(len(data) * (train_ratio + valid_ratio)) : int(
                len(data) * (train_ratio + valid_ratio + test_ratio)
            )
        ]
    ...
    print(
        ...
        str(test_end_time),
        "; Gap: ",
        str(test_end_time - test_start_time),
    )
    ...
    interaction_timestamp, recent_timestamp = prepare_interaction_time(
        train_data, num_nodes, device
    )
    ...
    print(len(all_nodes))
    head_node2candidate, tail_node2candidate = get_node2candidate(
        train_data, all_nodes, pri
    )
    for interaction in test_data:
        ...
        nor_ = nor
        if nor_ == 1:
            emb_emb = nn.Embedding.from_pretrained(
                nn.functional.normalize(emb_emb.weight)
            )
        ...
        if (line == 0) & (transfer == 1):
            head_reps = nn.Embedding.from_pretrained(
                dyGnn.transfer2head(emb_emb.weight)
            )
            tail_reps = nn.Embedding.from_pretrained(
                dyGnn.transfer2tail(emb_emb.weight)
            )

        if nor_ == 2:
            head_reps = nn.Embedding.from_pretrained(
                nn.functional.normalize(head_reps.weight)
            )
            tail_reps = nn.Embedding.from_pretrained(
                nn.functional.normalize(tail_reps.weight)
            )

        bo = args.bo
        if bo:
            if skip:
                head_rank, tail_rank, head_length, tail_length = get_rank_new(
                    interaction,
                    head_reps,
                    tail_reps,
                    device,
    ...
    print(
        "head_length mean: ",
        np.mean(head_lengths_numpy),
        ";",
        "num_test: ",
        head_lengths_numpy.shape[0],
    )
    print(
        "tail_lengths mean: ",
        np.mean(tail_lengths_numpy),
        ";",
        "num_test: ",
        tail_lengths_numpy.shape[0],
    )
    print(
        "head_rank mean: ",
        np.mean(head_ranks_numpy),
        " ; ",
        "head_rank var: ",
        np.var(head_ranks_numpy),
    )
    print(
        "tail_rank mean: ",
        np.mean(tail_ranks_numpy),
        " ; ",
        "tail_rank var: ",
        np.var(tail_ranks_numpy),
    )
    ...
    print(
        "Recall_20: ",
        ((head_ranks_numpy <= 20).sum() + (tail_ranks_numpy <= 20).sum())
        / head_lengths_numpy.shape[0]
        / 2,
    )
    print(
        "Recall_50: ",
        ((head_ranks_numpy <= 50).sum() + (tail_ranks_numpy <= 50).sum())
        / head_lengths_numpy.shape[0]
        / 2,
    )
...
def main():
    ...
    if args.dataset == "uci":
        data = Temporal_Dataset(
            "Dataset/UCI_email_1899_59835/opsahl-ucsocial/out.opsahl-ucsocial", 1, 2
        )
        num_nodes = 1899
