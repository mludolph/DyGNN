...
from src.decayer import Decayer
from src.edge_updater import Edge_updater_nn
from src.node_updater import TLSTM
...
class DyGNN(nn.Module):
    def __init__(
        ...
        self.edge_updater_head = Edge_updater_nn(
            embedding_dims, edge_output_size, act, relation_size
        ).to(device)
        self.edge_updater_tail = Edge_updater_nn(
            embedding_dims, edge_output_size, act, relation_size
        ).to(device)

        if if_no_time:
            self.node_updater_head = nn.LSTMCell(
                edge_output_size, embedding_dims, bias
            ).to(device)
            self.node_updater_tail = nn.LSTMCell(
                edge_output_size, embedding_dims, bias
            ).to(device)
        else:
        ...
        self.tran_head_edge_head = nn.Linear(edge_output_size, embedding_dims, bias).to(
            device
        )
        self.tran_head_edge_tail = nn.Linear(edge_output_size, embedding_dims, bias).to(
            device
        )

        self.tran_tail_edge_head = nn.Linear(edge_output_size, embedding_dims, bias).to(
            device
        )
        self.tran_tail_edge_tail = nn.Linear(edge_output_size, embedding_dims, bias).to(
            device
        )
        self.is_att = is_att
        if self.is_att:
        ...
        self.recent_timestamp = torch.zeros(
            (num_embeddings, 1), dtype=torch.float, requires_grad=False
        ).to(device)
        ...
        self.hidden_head = nn.Embedding(num_embeddings, embedding_dims, weight).to(
            device
        )
        self.hidden_head.weight.requires_grad = False
        self.hidden_tail = nn.Embedding(num_embeddings, embedding_dims, weight).to(
            device
        )
        self.hidden_tail.weight.requires_grad = False

        self.node_representations = nn.Embedding(
            num_embeddings, embedding_dims, weight
        ).to(device)
        self.node_representations.weight.requires_grad = False

        if transfer:
            self.transfer2head = nn.Linear(embedding_dims, embedding_dims, False).to(
                device
            )
            self.transfer2tail = nn.Linear(embedding_dims, embedding_dims, False).to(
                device
            )
            if drop_p >= 0:
    ...
    def reset_time(self):
        self.recent_timestamp = torch.zeros(
            (self.num_embeddings, 1), dtype=torch.float, requires_grad=False
        ).to(self.device)
        self.interaction_timestamp = lil_matrix(
            (self.num_embeddings, self.num_embeddings), dtype=np.float32
        )

    def reset_reps(self):
        self.cell_head = nn.Embedding.from_pretrained(
            self.cell_head_copy.weight.clone()
        ).to(self.device)
        self.cell_tail = nn.Embedding.from_pretrained(
            self.cell_tail_copy.weight.clone()
        ).to(self.device)
        self.hidden_head = nn.Embedding.from_pretrained(
            self.hidden_head_copy.weight.clone()
        ).to(self.device)
        self.hidden_tail = nn.Embedding.from_pretrained(
            self.hidden_tail_copy.weight.clone()
        ).to(self.device)
        self.node_representations = nn.Embedding.from_pretrained(
            self.node_representations_copy.weight.clone()
        ).to(self.device)
    ...
    def forward(self, interactions):
        ...
        for i in range(steps):
            ...
            if self.if_no_time:
                ...
                ) = self.node_updater_head(
                    edge_info_head, (head_node_hidden_head, head_node_cell_head)
                )
            else:
                (
                    updated_head_node_cell_head,
            ...
            if self.if_no_time:
                (
                    updated_tail_node_hidden_tail,
                    updated_tail_node_cell_tail,
                ) = self.node_updater_tail(
                    edge_info_tail, (tail_node_hidden_tail, tail_node_cell_tail)
                )
            else:
                (
                    updated_tail_node_cell_tail,
            ...
            if self.if_propagation:
                head_node_head_neighbors, head_node_tail_neighbors = self.propagation(
                    head_index,
                    current_t,
                    edge_info_head,
                    "head",
                ...
                tail_node_head_neighbors, tail_node_tail_neighbors = self.propagation(
                    tail_index,
                    current_t,
                    edge_info_tail,
                    "tail",
                ...
                (
                    head_node_head_neighbors,
                    head_node_tail_neighbors,
                    n_i_1,
                    n_i_2,
                ) = self.get_neighbors(head_index, current_t, self.threhold)
                (
                    tail_node_head_neighbors,
                    tail_node_tail_neighbors,
                    n_i_1,
                    n_i_2,
                ) = self.get_neighbors(tail_index, current_t, self.threhold)
            ...
            all_head_nodes = (
                all_head_nodes | head_node_head_neighbors | tail_node_head_neighbors
            )
            all_tail_nodes = (
                all_tail_nodes | head_node_tail_neighbors | tail_node_tail_neighbors
            )

            # generate negative samples
            tail_candidates = (
                all_tail_nodes - {head_index, tail_index} - head_node_tail_neighbors
            )
            if len(tail_candidates) == 0:
                tail_neg_samples = list(
                    choice(range(self.num_embeddings), size=self.num_negative)
                )

            else:
                tail_neg_samples = list(
                    choice(list(tail_candidates), size=self.num_negative)
                )

            head_candidates = (
                all_head_nodes - {tail_index, head_index} - tail_node_head_neighbors
            )
            if len(head_candidates) == 0:
                head_neg_samples = list(
                    choice(range(self.num_embeddings), size=self.num_negative)
                )
            else:
                head_neg_samples = list(
                    choice(list(head_candidates), size=self.num_negative)
                )
            ...
            self.interaction_timestamp[head_index, tail_index] = current_t[0, 0].cpu()
        ...
        output_cell_head_tensor = torch.cat([*output_cell_head]).view(
            -1, self.embedding_dims
        )
        output_hidden_head_tensor = torch.cat([*output_hidden_head]).view(
            -1, self.embedding_dims
        )
        output_rep_head_tensor = torch.cat([*output_rep_head]).view(
            -1, self.embedding_dims
        )

        output_cell_tail_tensor = torch.cat([*output_cell_tail]).view(
            -1, self.embedding_dims
        )
        output_hidden_tail_tensor = torch.cat([*output_hidden_tail]).view(
            -1, self.embedding_dims
        )
        output_rep_tail_tensor = torch.cat([*output_rep_tail]).view(
            -1, self.embedding_dims
        )
        ...
        if self.transfer:
            output_rep_head_tensor = self.dropout(
                self.transfer2head(output_rep_head_tensor)
            )
            output_rep_tail_tensor = self.dropout(
                self.transfer2tail(output_rep_tail_tensor)
            )
        ...
        return (
            output_rep_head_tensor,
            output_rep_tail_tensor,
            head_neg_tensors,
            tail_neg_tensors,
        )
    ...
    def propagation(
        ...
        (
            ...
            head_timestamps,
            tail_timestamps,
        ) = self.get_neighbors(node, current_t, threhold)
        ...
        if len(head_neighbors) > 0:
            ...
            head_delta_ts = current_t.repeat(
                len(head_timestamps), 1
            ) - torch.FloatTensor(head_timestamps).to(self.device).view(-1, 1)
            transed_head_delta_ts = self.decayer(head_delta_ts)

            head_nei_cell = self.get_rep(head_neighbors, "cell_head", node2cell_head)
            if self.if_no_time:
                tran_head_nei_edge_info = head_nei_edge_info.repeat(
                    len(head_neighbors), 1
                )
            else:
                tran_head_nei_edge_info = (
                    head_nei_edge_info.repeat(len(head_neighbors), 1)
                    * transed_head_delta_ts
                )
            ...
            head_nei_tail_hidden = self.get_rep(
                head_neighbors, "hidden_tail", node2hidden_tail
            )
        ...
        if len(tail_neighbors) > 0:
            ...
            tail_delta_ts = current_t.repeat(
                len(tail_timestamps), 1
            ) - torch.FloatTensor(tail_timestamps).to(self.device).view(-1, 1)
            transed_tail_delta_ts = self.decayer(tail_delta_ts)

            tail_nei_cell = self.get_rep(tail_neighbors, "cell_tail", node2cell_tail)
            if self.if_no_time:
                tran_tail_nei_edge_info = tail_nei_edge_info.repeat(
                    len(tail_neighbors), 1
                )
            else:
                tran_tail_nei_edge_info = (
                    tail_nei_edge_info.repeat(len(tail_neighbors), 1)
                    * transed_tail_delta_ts
                )
            ...
            tail_nei_head_hidden = self.get_rep(
                tail_neighbors, "hidden_head", node2hidden_head
            )
    ...
    def loss(self, interactions):
        ...
        (
            ...
            head_neg_tensors,
            tail_neg_tensors,
        ) = self.forward(interactions)

        head_pos_tensors = (
            output_rep_head_tensor.clone()
            .repeat(1, self.num_negative)
            .view(-1, self.embedding_dims)
        )
        tail_pos_tensors = (
            output_rep_tail_tensor.clone()
            .repeat(1, self.num_negative)
            .view(-1, self.embedding_dims)
        )
        ...
        labels_n = torch.FloatTensor([0] * num_pp * 2 * self.num_negative).to(
            self.device
        )
        ...
        scores_p = torch.bmm(
            output_rep_head_tensor.view(num_pp, 1, self.embedding_dims),
            output_rep_tail_tensor.view(num_pp, self.embedding_dims, 1),
        )
        scores_n_1 = torch.bmm(
            head_neg_tensors.view(num_pp * self.num_negative, 1, self.embedding_dims),
            tail_pos_tensors.view(num_pp * self.num_negative, self.embedding_dims, 1),
        )
        scores_n_2 = torch.bmm(
            head_pos_tensors.view(num_pp * self.num_negative, 1, self.embedding_dims),
            tail_neg_tensors.view(num_pp * self.num_negative, self.embedding_dims, 1),
        )

        scores = torch.cat((scores_p, scores_n_1, scores_n_2)).view(
            num_pp * (1 + 2 * self.num_negative)
        )
